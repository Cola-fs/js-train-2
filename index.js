...
function getNumberFromString(s) {
  let chislo = parseFloat(s);

  let result = "";
  if (isNaN(chislo)) {
    result = "Couldn't extract a number from the provided string.";
  } else {
    result = chislo;
  }
...
function isNumberFinite(num) {
  let result = "";
  if (isFinite(num)) {
    result = "The number is finite.";
  } else {
    result = "The number is not finite.";
  }
  return result;
...
function isValueNaN(value) {
  let result = '';
  if (isNaN(value))
  {result = "The value is NaN."}
  else
  {result = "The value is not NaN."};
  return result;
